...
def main(args):
    ...
    plot_dic = {
        ...
        "linewidth": 1,
        "alpha": 0.8,
        "color": {token_name: plt.cm.tab20.colors[i % 20] for i, token_name in enumerate(token_names)}, # token_nameごとに異なる色を割り当てる
    ...
    if args.pca_components == 2:
        output_path += ".png"
        plot_trajectory_2d(Y, token_names, steps, conceptForFict2token_map, output_path, plot_dic, xlim=(-5, 5), ylim=(-4.5, 4.5))
    elif args.pca_components == 3:
        output_path += ".html"
        plot_trajectory_3d(Y, token_names, steps, conceptForFict2token_map, output_path, plot_dic)
    else:
...
def plot_trajectory_2d(vectors_2d, token_names, steps, conceptForFict2token_map, output_path, plot_dic, xlim=None, ylim=None):
    ...
    for i, token_name in enumerate(token_names):
        ...
        plt.plot(
            ...
            label=f"{token_name}",   # f"token {token_id} (originally {concept})")
            color=plot_dic["color"][token_name]
        )
...
def plot_trajectory_3d(vectors_3d, token_names, steps, conceptForFict2token_map, output_path, plot_dic):
    # =========================
    # DataFrame化
    # =========================
    token_labels = np.repeat([f"{t}" for t in token_names], len(steps))
    ...
    df = pd.DataFrame({
        ...
        "token": np.repeat([f"{t}" for t in token_names], len(steps)),
        "step": np.tile(steps, len(token_names)),
        # "color": np.repeat(
