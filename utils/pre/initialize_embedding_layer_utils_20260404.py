    ...
    def initializeEmbed(
        ...
        * どのモデルでも共通のはず
        ...
        handlers = {
            ...
            "category_centroid_by_hidden_state_mean": lambda: self.initvec_by_category_centroid_by_hidden_state_mean(
                model, tokenizer, category_to_concepts_for_vec, category2initoken_ids,
                layer_idx=layer_idx, print_flag=print_flag
            ),
            ...
            "other_category_centroid_by_hidden_state_mean": lambda: self.initvec_by_other_category_centroid_by_hidden_state_mean(
                model, tokenizer, category_to_concepts_for_vec, category2initoken_ids,
                layer_idx=layer_idx, print_flag=print_flag
            ),
            ...
            # vec_propnoun = h_last_token_in_propnoun -> 各カテゴリの初期化vec = mean(vec_propnoun_in_concept)
            "categoryCentroid_by_DebiasedHiddenState": lambda: self.initvec_by_category_centroid_by_debiased_hidden_state(
                model, tokenizer, category_to_concepts_for_vec, category2initoken_ids,
                layer_idx=layer_idx, print_flag=print_flag
            ),
            # *** (Type3) 他のカテゴリのCOGで初期化. category_centroid_by_debiased_hidden_state の対。***
            "otherCategoryCentroid_by_DebiasedHiddenState": lambda: self.initvec_by_other_category_centroid_by_debiased_hidden_state(
                model, tokenizer, category_to_concepts_for_vec, category2initoken_ids,
                layer_idx=layer_idx, print_flag=print_flag
            ),
            ...
            "categoryCentroid_by_DebiasedHSMixed": lambda: self.initvec_by_category_centroid_by_debiased_and_mixed_hidden_state(
                model, tokenizer, category_to_concepts_for_vec, category2initoken_ids,
                layer_idx, print_flag
            ),
            # *** (Type4) 他のカテゴリのCOGで初期化. category_centroid_by_debiased_hidden_state の対。***
            "otherCategoryCentroid_by_DebiasedHSMixed": lambda: self.initvec_by_other_category_centroid_by_debiased_and_mixed_hidden_state(
                model, tokenizer, category_to_concepts_for_vec, category2initoken_ids,
                layer_idx, print_flag
            ),
            ...
            "CatCentroid_by_OthCatDebiasedHSMixed": lambda: self.initvec_by_category_centroid_by_global_vec_debiased_and_mixed_hidden_state(
                model, tokenizer, category_to_concepts_for_vec, category2initoken_ids,
                layer_idx, print_flag
            ),
            # *** Type5の対 ***
            "otherCatCentroid_by_OthCatDebiasedHSMixed": lambda: self.initvec_by_other_category_centroid_by_global_vec_debiased_and_mixed_hidden_state(
                model, tokenizer, category_to_concepts_for_vec, category2initoken_ids,
                layer_idx, print_flag
            ),
            ...
            # * 前後3層の隠れ状態を平均する
            "CatCent_by_GlbPrimDebiasedHSMixed": lambda: self.initvec_by_category_centroid_by_global_vec_debiased_and_mixed_hidden_state(
                model, tokenizer, category_to_concepts_for_vec, category2initoken_ids,
                layer_idx, print_flag
            ),
            "otherCatCent_by_GlbPrimDebiasedHSMixed": lambda: self.initvec_by_other_category_centroid_by_global_vec_debiased_and_mixed_hidden_state(
                model, tokenizer, category_to_concepts_for_vec, category2initoken_ids,
                layer_idx, print_flag
            ),

            # * 単一層の隠れ状態を使う
            "CatCent_by_GlbPrimDebiasedHS": lambda: self.initvec_by_category_centroid_by_global_vec_debiased_hidden_state(
                model, tokenizer, category_to_concepts_for_vec, category2initoken_ids,
                layer_idx, print_flag
            ),
            "otherCatCent_by_GlbPrimDebiasedHS": lambda: self.initvec_by_other_category_centroid_by_global_vec_debiased_hidden_state(
                model, tokenizer, category_to_concepts_for_vec, category2initoken_ids,
                layer_idx, print_flag
            ),
            ...
            # * 前後3層の隠れ状態を平均する
            "CatCent_by_WikiSummaryHSMixed": lambda: self.initvec_by_category_centroid_by_wiki_summary_and_mixed_hidden_state(
                model, tokenizer, category_to_concepts_for_vec, category2initoken_ids,
                layer_idx, print_flag
            ),
...
# mix_layers initVecWithGlobalVecDebiasedTermHS
    ...
    # ***
    def initvec_by_category_centroid_by_hidden_state_mean(self, model, tokenizer, category_to_concepts_for_vec, category2initoken_ids, layer_idx=-1, print_flag=False):
        # *** (Type2) 各概念のベクトルを、同一カテゴリ内の固有名詞のベクトルの平均(カテゴリの重心と考える)で初期化する方法. ***
        # Type2: 各prop nounをモデルに入力し、語句内の最終token位置における指定層の隠れ状態をその固有名詞のベクトルとする方法。
        # vec_propnoun = h_last_token_in_propnoun -> 各カテゴリの初期化vec = mean(vec_propnoun_in_concept)
    ...
    def initvec_by_other_category_centroid_by_hidden_state_mean(self, model, tokenizer, category_to_concepts_for_vec, category2initoken_ids, layer_idx=-1, print_flag=False):
        # *** (Type2) 他のカテゴリのCOGで初期化. category_centroid_by_hidden_state_mean の対。例えば、動物カテゴリの新規概念を、場所カテゴリの固有名詞のベクトルの平均で初期化するなど. ***
        # * カテゴリ毎のcentroid vec作成用の固有名詞リスト(propnoun_num_for_init_vec-10 個)を作成
    ...
    # ***
    def initvec_by_category_centroid_by_debiased_hidden_state(self, model, tokenizer, category_to_concepts_for_vec, category2initoken_ids, layer_idx=-1, print_flag=False):
        """ (Type3) 各概念のベクトルを、同一カテゴリ内の固有名詞のベクトルの平均(カテゴリの重心と考える)で初期化する方法. 
            更に、等方性(異方性?)を解消し、カテゴリ間のvecが類似することを防ぐため、生の hidden state 平均ではなく、中心化([WIP]・白色化)してから centroid を作る。
        方法: 
            * 収集した全固有名詞 の hidden state 平均(global_hidden_mean)を引く. term_vec = term_vec - global_hidden_mean
    ...
    def initvec_by_other_category_centroid_by_debiased_hidden_state(self, model, tokenizer, category_to_concepts_for_vec, category2initoken_ids, layer_idx=-1, print_flag=False):
        # *** (Type3) 他のカテゴリのCOGで初期化. category_centroid_by_debiased_hidden_state の対。***
        category_to_centroid_terms = {}
        for category, init_token_ids in category2initoken_ids.items():
            init_terms_candidate = category_to_concepts_for_vec[category]
    ...
    def initvec_by_category_centroid_by_debiased_and_mixed_hidden_state(self, model, tokenizer, category_to_concepts_for_vec, category2initoken_ids, layer_idx, print_flag):
        """ (Type4) 各概念のベクトルを、同一カテゴリ内の固有名詞のベクトルの平均(カテゴリの重心と考える)で初期化する方法. 
            更に、等方性(異方性?)を解消し、カテゴリ間のvecが類似することを防ぐため、生の hidden state 平均ではなく、中心化([WIP]・白色化)してから centroid を作る。
        方法: 
            * 収集した全固有名詞 の hidden state 平均(global_hidden_mean)を引く. term_vec = term_vec - global_hidden_mean
        ...
        for category, init_token_ids in category2initoken_ids.items():
            ...
            for init_token_id in init_token_ids:
                ...
                # 埋め込み層のinit_token_id行を、init_termsの指定層における隠れ状態の平均で初期化する
                model = self.initVecWithGlobalVecDebiasedTermHS(model, tokenizer, category, init_terms, [init_token_id], layer_idx=layer_idx, lambda_=LAMBDA_, mix_layers=True, print_flag=print_flag)
                print(f"Initialized new token {tokenizer.decode(init_token_id)} in category '{category}' with layer {layer_idx}'s hidden state of {len(init_terms)} concepts: ... ({init_terms[-15:]}).") 
        return model

    
    def initvec_by_other_category_centroid_by_debiased_and_mixed_hidden_state(self, model, tokenizer, category_to_concepts_for_vec, category2initoken_ids, layer_idx, print_flag):
        # *** (Type4) 他のカテゴリのCOGで初期化. category_centroid_by_debiased_hidden_state の対。***
        category_to_centroid_terms = {}
        for category, init_token_ids in category2initoken_ids.items():
            init_terms_candidate = category_to_concepts_for_vec[category]
        ...
        for category, init_token_ids in category2initoken_ids.items():
            ...
            for init_token_id in init_token_ids:
                ...
                model = self.initVecWithGlobalVecDebiasedTermHS(
                    model, tokenizer, 
                    category,   # own_caetgoryは、global_vec として使うカテゴリを選択するために指定する。init_vecを計算する固有名詞の選択には使われない。 
                    init_terms, [init_token_id], layer_idx=layer_idx, lambda_=LAMBDA_, mix_layers=True, print_flag=print_flag)
                print(f"Initialized new token {tokenizer.decode(init_token_id)} in category '{category}' with layer {layer_idx}'s hidden state of {len(init_terms)} concepts: ... ({init_terms[-15:]}), from other category '{other_category}'.")
        return model

    
    def initvec_by_category_centroid_by_global_vec_debiased_and_mixed_hidden_state(self, model, tokenizer, category_to_concepts_for_vec, category2initoken_ids, layer_idx, print_flag):
        """ 各概念のベクトルを、同一カテゴリ内の固有名詞のベクトルの平均(カテゴリの重心と考える)で初期化する方法. 
            更に、等方性(異方性?)を解消し、カテゴリ間のvecが類似することを防ぐため、生の hidden state 平均ではなく、中心化([WIP]・白色化)してから centroid を作る。
        方法: 
            * 収集した全固有名詞 の hidden state 平均(global_hidden_mean)を引く. term_vec = term_vec - global_hidden_mean
        ...
        for own_category, init_token_ids in category2initoken_ids.items():
            ...
            for init_token_id in init_token_ids:
                ...
                # 埋め込み層のinit_token_id行を、init_termsの指定層における隠れ状態の平均で初期化する
                # model = self.initVecWithGlobalVecDebiasedTermHS(model, tokenizer, own_category, init_terms, [init_token_id], layer_idx=layer_idx, lambda_=LAMBDA_, init_vec_type=init_vec_type, mix_layers=True, print_flag=print_flag)
                model = self.initVecWithGlobalVecDebiasedTermHS(model, tokenizer, own_category, init_terms, [init_token_id], layer_idx=layer_idx, lambda_=LAMBDA_, mix_layers=True, print_flag=print_flag)
    ...
    def initvec_by_other_category_centroid_by_global_vec_debiased_and_mixed_hidden_state(self, model, tokenizer, category_to_concepts_for_vec, category2initoken_ids, layer_idx, print_flag):
        # *** initvec_by_category_centroid_by_global_vec_debiased_and_mixed_hidden_state の対。***
    ...
    def initvec_by_category_centroid_by_global_vec_debiased_hidden_state(self, model, tokenizer, category_to_concepts_for_vec, category2initoken_ids, layer_idx, print_flag):
        """ 各概念のベクトルを、同一カテゴリ内の固有名詞のベクトルの平均(カテゴリの重心と考える)で初期化する方法. 
            更に、等方性(異方性?)を解消し、カテゴリ間のvecが類似することを防ぐため、生の hidden state 平均ではなく、中心化([WIP]・白色化)してから centroid を作る。
        方法: 
            * 収集した全固有名詞 の hidden state 平均(global_hidden_mean)を引く. term_vec = term_vec - global_hidden_mean
        ...
        for own_category, init_token_ids in category2initoken_ids.items():
            ...
            for init_token_id in init_token_ids:
                ...
                model = self.initVecWithGlobalVecDebiasedTermHS(model, tokenizer, own_category, init_terms, [init_token_id], layer_idx=layer_idx, lambda_=LAMBDA_, mix_layers=False, print_flag=print_flag)
                print(f"Initialized new token {tokenizer.decode(init_token_id)} in category '{own_category}' with layer {layer_idx}'s hidden state of {len(init_terms)} concepts: ... ({init_terms[-15:]}).") 
        return model


    def initvec_by_other_category_centroid_by_global_vec_debiased_hidden_state(self, model, tokenizer, category_to_concepts_for_vec, category2initoken_ids, layer_idx, print_flag):
        # *** (Type4) 他のカテゴリのCOGで初期化. category_centroid_by_debiased_hidden_state の対。***
        # 準備: 各カテゴリのcentroid vec作成用の固有名詞リスト(propnoun_num_for_init_vec-10 個)を作成しておく
        category_to_centroid_terms = {}
        for category, init_token_ids in category2initoken_ids.items():
        ...
        for own_category, init_token_ids in category2initoken_ids.items():
            ...
            for init_token_id in init_token_ids:
                ...
                # model = self.initVecWithGlobalVecDebiasedTermHS(model, tokenizer, own_category, init_terms, [init_token_id], layer_idx=layer_idx, lambda_=0.1, init_vec_type=self.init_vec_type,  mix_layers=False, print_flag=print_flag)
                model = self.initVecWithGlobalVecDebiasedTermHS(model, tokenizer, own_category, init_terms, [init_token_id], layer_idx=layer_idx, lambda_=LAMBDA_, mix_layers=False, print_flag=print_flag)
                print(f"Initialized new token {tokenizer.decode(init_token_id)} in category '{own_category}' with layer {layer_idx}'s hidden state of {len(init_terms)} concepts: ... ({init_terms[-15:]}), from other category '{other_category}'.")
        return model
    ...
    def initvec_by_category_centroid_by_wiki_summary_and_mixed_hidden_state(self, model, tokenizer, category_to_concepts_for_vec, category2initoken_ids, layer_idx, print_flag):
        # *** 初期vecを、カテゴリの固有名詞ではなく、wikiのsummary文を入力した時の最終tokenの隠れ状態から作る。***
        # 固有名詞を構成する単語は、別の意味を持っていることが多いのではないかという懸念から。例えば、board_gameカテゴリの'Unlock!'というゲーム名は、単語だけ見れば鍵を開けるという意味だが、ここではゲームを代表するベクトルを作りたいため、単語の意味と欲しい意味が異なる。そのためwiki説明文の利用を試す。
