...
class EmbedInitializer:
    ...
    def calculateGlobalHSMean_by_GlbPrimDebiasedHSMixed(self, model, tokenizer, n_components, mix_layers):
        ...
        # dbpediaから収集した全ての固有名詞を収集
        # propNoun_dir = os.path.join(project_root, "data", "dbpedia", "wikidata_Things_childs_LIMIT1000")
        # prop_nouns = []  # all_prop_nouns = []
        # for category_file in os.listdir(propNoun_dir):
        #     if category_file.endswith(".csv"):
        #         df = pd.read_csv(os.path.join(propNoun_dir, category_file))
        #         # 全部追加すると多すぎたので、各カテゴリからランダムに100個だけ追加することにする
        #         prop_nouns += random.sample(df['label'].tolist(), min(self.num_propNouns_in_cat_for_globalHSMean, len(df)))
    ...
    def initializeEmbed(
        ...
        * どのモデルでも共通のはず
        ...
        if init_vec_type == 'uniform':
            # ** 一様分布で初期化 **
            model = self.initvec_by_uniform(model, train_token2tokenid)
            return model

        elif init_vec_type == 'norm_rand':
            # ** ノルム固定の正規化ランダムで初期化 **
            model = self.initvec_by_norm_rand(model, train_token2tokenid)
            return model

        elif init_vec_type == 'norm_rand_vocab':
            # ** 正規化ランダムで初期化（N(μ,σ2)のμとσは語彙集合から計算） **
            model = self.initvec_by_norm_rand_vocab(model, train_token2tokenid)
            return model

        elif init_vec_type == 'zero':
            # ** 0vecで初期化 **
            model = self.initvec_by_zero(model, train_token2tokenid)
            return model
        ...
        # ============================================================================================
            ...
            model = self.initvec_by_other_category_centroid_plus_random(
                model, 
                tokenizer, 
                category_to_concepts_for_vec, 
                category2initoken_ids, 
        ...
        # ============================================================================================
            ...
            model = self.initvec_by_category_COG_by_simple_mean(
            ...
            model = self.initvec_by_other_category_COG_by_simple_mean(
                model, tokenizer, category_to_concepts_for_vec, category2initoken_ids, print_flag=print_flag
            )
            return model
        ...
        # 2026/03/19
            ...
            model = self.initvec_by_category_centroid_by_hidden_state_mean( 
                model, tokenizer, category_to_concepts_for_vec, category2initoken_ids, layer_idx=layer_idx, print_flag=print_flag
            )
            return model
        

        # [WIP] ifぶん→関数に移植中2026/03/27
            ...
            model = self.initvec_by_other_category_centroid_by_hidden_state_mean(
                model, tokenizer, category_to_concepts_for_vec, category2initoken_ids, layer_idx=layer_idx, print_flag=print_flag
            )
            return model
        ...
        # 2026/03/20
            ...
            vec_propnoun = h_last_token_in_propnoun -> 各カテゴリの初期化vec = mean(vec_propnoun_in_concept)
            """
            model = self.initvec_by_category_centroid_by_debiased_hidden_state(
                model, tokenizer, category_to_concepts_for_vec, category2initoken_ids, layer_idx=layer_idx, print_flag=print_flag
            )
            return model
            ...
            model = self.initvec_by_other_category_centroid_by_debiased_hidden_state(
                model, tokenizer, category_to_concepts_for_vec, category2initoken_ids, layer_idx=layer_idx, print_flag=print_flag
            )
            return model
