...
def train(
    ...
    for epoch in tqdm(range(maxEpochs)): # [memo] for epoch in tqdm(range(maxEpochs + 1)): maxEpochs=10なら10epochまで学習させたかったが不自然らしいの+1を消した
        ...
        for step, i in enumerate(range(0, len(indices), BATCH_SIZE)):
            ...
            totalLoss += loss.item()    #  += loss.detach().cpu().tolist()
            total_steps += 1
        ...
        # =========================
        # 検証とschedulerの更新とearly stoppingの判定
        # =========================
        # [WIP] 検証データはないため，train lossでschedulerを更新する. 余裕があれば検証データ（別ルールによるtripletの言い換え)を作り，それをevaluateModelに入れてlossを計算するようにする
...
if __name__ == "__main__":
    ...
    for seed in range(args.seed_num):
        ...
        for init_vec_type in init_vec_type_lst:
            ...
            if layer_indices is None or len(layer_indices) < 1 or not need_layer_flag:
