...
def initializeEmbed(
    ...
    * どのモデルでも共通のはず
    ...
    if init_vec_type == 'uniform':
        ...
        # *** (Type2) 他のカテゴリのCOGで初期化. 例えば、動物カテゴリの新規概念を、場所カテゴリの固有名詞のベクトルの平均で初期化するなど. category_COGに対する比較用 ***
        ...
        for category, init_token_ids in category2initoken_ids.items():
            ...
            for init_token_id in init_token_ids:
                ...
                init_terms_for_centroid = category_to_centroid_terms[other_category]
...
def main(args):
    ...
    if layer_idx is not None and init_vec_type in ['category_centroid_by_hidden_state_mean', 'category_COG_by_hidden_state_mean']:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--layer_idx', type=int, default=None, help='隠れ状態を取得する層のインデックス。-1なら最終層、0以上の整数ならその層の隠れ状態を使用する。init_vec_typeが \'category_centroid_by_hidden_state_mean\' の場合に使用')
