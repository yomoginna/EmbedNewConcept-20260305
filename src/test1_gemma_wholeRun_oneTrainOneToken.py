...
"""
1conceptずつ訓練・保存したモデルに対して、test1を実施し、logitスコアを保存するコード
"""
...
def main(args):
    ...
    if init_vec_type in ['category_COG', 'other_category_COG']:
        mem_dir = os.path.join(project_root, "memvec_models", f"{model_name_for_dirname}_{target_concepts_filename.replace('.json', '')}_initvecwith{init_vec_type.replace(' ', '_')}") + '_5'
        result_dir = os.path.join(project_root, "results", f"{model_name_for_dirname}_{target_concepts_filename.replace('.json', '')}_initvecwith{init_vec_type.replace(' ', '_')}") + '_5'
    elif init_vec_type in ['zero', ]:
    ...
    # target_concept_listから1conceptずつtestする
    for target_concept in target_concepts:
        mem_dir_for_concept = os.path.join(mem_dir, target_concept.replace(' ', '_'))
        result_dir_for_concept = os.path.join(result_dir, target_concept.replace(' ', '_'))


        # ********* test1 *********
        epoch_list = []
        for filename in os.listdir(mem_dir_for_concept):
        ...
        for epoch in tqdm(epoch_list, desc="Evaluating test1 over epochs"):
            ...
            # このepochに対するtest1結果が存在する場合はスキップ
            skip_epoch = True
            logit_score_path = os.path.join(result_dir_for_concept, f"logit-scored_epoch{epoch}.json")
            if os.path.exists(logit_score_path):
                print(f"test1 result for concept '{target_concept}' at epoch {epoch} already exists at {logit_score_path}. Skipping this epoch.")
                continue
            ...
            if epoch == 0:
                ...
                # ** memvecをmodelに挿入・置換 **
                try:
                    mem_save_path = os.path.join(mem_dir_for_concept, f'{epoch}.pth.npy')
            ...
            # ****** test1 evaluation ******
            test1_lst = concept_to_test1_lst[target_concept]
            ...
            # logitを保存
            result_path = os.path.join(result_dir_for_concept, f"logit-scored_epoch{epoch}.json")
            os.makedirs(os.path.dirname(result_path), exist_ok=True)
            with open(result_path, 'w') as f:
                json.dump(results, f, indent=4)

        # *** 全epochのscoreを計算し，summaryとして保存 ***
        ...
        for epoch in epoch_list:
            ...
            epoch_to_results[epoch] = metrics
        ...
        with open(os.path.join(result_dir_for_concept, "logit_score_summary.json"), 'w') as f:
            json.dump(epoch_to_results, f, indent=4)

    # *** 全conceptの全epochのscoreをまとめたsummaryを保存 (不要ではあるが) ***
    concept_to_results = {}
    for concept in concept_to_test1_lst.keys():
        logit_score_summary_path = os.path.join(result_dir, concept.replace(' ', '_'), "logit_score_summary.json")
        if os.path.exists(logit_score_summary_path):
            with open(logit_score_summary_path, 'r') as f:
                concept_to_results[concept] = json.load(f)
...
if __name__ == "__main__":
    ...
    for seed in range(args.seed_num):
        ...
        # 通常:
        # init_vec_type_lst = ['category_COG', 'zero', 'uniform', 'norm_rand', 'norm_rand_vocab']
        # init_vec_type_lst = ['category_COG', 'other_category_COG', 'zero', 'uniform']
