"""
カテゴリ内の全トークンを一度に訓練すると、accがかなり下がったので、1トークンずつ訓練するバージョンも作成することにした。
[WIP] 1カテゴリあたり1トークンをまとめて訓練するのでも良いかもしれない。
"""
# ===== Standard library =====
import argparse
...
def initializeEmbed(
        model, 
        tokenizer, 
        train_token_id,     # train_token2tokenid, 
        target_category, # category2initoken_ids,
        init_vec_type, 
    ...
    * どのモデルでも共通のはず
    ...
    if init_vec_type == 'uniform':
        ...
        init_terms = category_to_concepts_for_vec[target_category]
        # 埋め込み層のinit_token_idsに該当する行を、init_termsのtokenベクトルの平均で初期化する
        model = initVecWithTokenVec(
            model, 
            tokenizer, 
            init_terms, 
            train_token_id,     # init_token_ids, 
            print_flag=print_flag
        )
        print(f"Initialized category '{target_category}' (new token {train_token_id}) with {len(init_terms)} concepts ({init_terms[:5]}...) for token {tokenizer.decode(train_token_id)}.")
        ...
        other_categories = [c for c in category_to_concepts_for_vec.keys() if c != target_category]
        print(f"Category '{target_category}' will be initialized with COG of other categories: {other_categories[:5]}.")
        ...
        # if print_flag:
        print(f"Initialized category '{target_category}' (new token {train_token_id}) with {len(init_terms)} concepts ({init_terms[:5]}...) from other category '{other_category}' for token {tokenizer.decode(train_token_id)}.")
        return model

    else: 
        # ** 指定の語句で初期化 (句の場合は単純にmean poolingする) **
        init_terms = [init_vec_type]  # 'a chair' など
        model = initVecWithTokenVec(model, tokenizer, init_terms, train_token_id, print_flag=print_flag)
        return model



def initVecWithTokenVec(model, tokenizer, init_terms, train_token_id, print_flag=False):
    """
    ...
    # 4. 埋め込み層のtrain_token_idが指定した<unusedx>を、まとめてinit_srcで初期化. 
    with torch.no_grad():
        init_target_ids = torch.as_tensor([train_token_id], device=E.device, dtype=torch.long)#.view(-1)     # torch long型に変換
...
def prepareGemmaModel(
    ...
    * special_tokenは全て初期化し, 学習可能な状態にする
    ...
    try:
        ...
        embeddingsToKeep = [i for i in range(model.model.embed_tokens.weight.shape[0]) if i != train_token_id]
        ...
        embeddingsToKeep = [i for i in range(model.model.language_model.embed_tokens.weight.shape[0]) if i != train_token_id]
        gzh = GradZeroHook(embeddingsToKeep)
...
def main(args):
    ...
    save_mem_dir_root = os.path.join(project_root, "memvec_models", f"{model_name_for_dirname}_{target_concepts_filename.replace('.json', '')}_initvecwith{init_vec_type.replace(' ', '_')}")

    # もしすでに同名のディレクトリが存在していたら、_2のように末尾に連番をつける
    original_save_mem_dir_root = save_mem_dir_root
    counter = 2
    while os.path.exists(save_mem_dir_root):
        save_mem_dir_root = f"{original_save_mem_dir_root}_{counter}"
        counter += 1
    print('save_mem_dir_root:', save_mem_dir_root)
    os.makedirs(save_mem_dir_root, exist_ok=True)
    ...
    save_path = os.path.join(save_mem_dir_root, "token_assignment.json")
    ...
    # これは、'category_COG' 初期化の場合、categoryごとのvec初期化の際に、同じカテゴリの概念に割り当てたtokenのidをまとめて初期化するために使用する. 
    ...
    if init_vec_type in ['category_COG', 'other_category_COG']:
        ...
        # category_COG 以外の初期化の場合は不要なので、None
        category2initoken_ids = None


    # ************* [trainMemVec_fromXvec_gemma_wholeRun.pyからの変更点] *************
    # target_concept_listから1conceptずつ訓練し、訓練したparameterをconcept毎に保存する

    for target_concept in tqdm(target_concept_list, desc="Training concepts"):

        save_mem_dir = os.path.join(save_mem_dir_root, target_concept.replace(' ', '_'))
        ...
        wandb_dir = os.path.join(project_root, "memvec_wandb_logs", model_name_for_dirname, target_concept.replace(' ', '_'))
        set_wandb_env(PROJECT_NAME, wandb_dir)
        # urlを記録
        wandb_url_log_path = os.path.join(save_mem_dir, "wandb_url.txt")
        with open(wandb_url_log_path, "w") as f:
            f.write(f"WandB URL for this run: {wandb.run.get_url()}\n")


        # ********* model等の準備: 予約済み特殊トークンの埋め込みの初期化など *********
        train_token_id = train_token2tokenid[conceptForFict2token_map[target_concept]]
        target_category = conceptForFict2category_map[target_concept]
    ...
    print("All concepts have been trained.")
