...
SPARQL_1HOP_GET_SUBCLASSES_BASE ="""
...
def main():
    ...
    for category in categories:
        ...
        # *** まずは1hopクエリで、直下のサブクラスを取得する ***
        query = SPARQL_1HOP_GET_SUBCLASSES_BASE.format(category=category)

        data = run_sparql(query)
        bindings = data.get("results", {}).get("bindings", [])
        # if not bindings:
        #     print(f"No subclasses found for {category}, skipping...")
        #     continue
        ...
        # *** もし、直下のサブクラスがSUBCLASS_NUM_THRESHOLD未満であれば、Multi-hopクエリも実行 ***
        if len(rows) < SUBCLASS_NUM_THRESHOLD:
            print(f"\tOnly {len(rows)} subclasses found for {category} with 1-hop query, trying multi-hop query...")
