...
修正中のコード(2026/06/02)
- 必須の修正ではない．
- innitialize_embedding_layer_utils.pyで設定した make_initvec_by_wiki_summary_and_hidden_state() などの関数を直接使って，カテゴリ代表vecを作るよう変更したい．
- したいが，コードは正しいのでしなくても良い
"""
...
def main(args):
    ...
    # =========================
    # 各カテゴリ代表vecを作成する固有名詞リストの作成.
    # 学習時の初期化の場合と違うのは，学習時は新規tokenごとに使う既存概念リストを変えて初期化vecを作るが，
    # カテゴリ代表vec一つを作成する今回は，カテゴリごとに一つの固有名詞リストを作成し、そのリスト内の全ての固有名詞を使ってカテゴリ代表vecを作る点。
    # =========================
    category2properNouns = {}
    for cat in target_cats:
        ...
        if len(propnouns) < num_nouns_per_category:
            ...
            continue
        category2properNouns[cat] = propnouns


    # ========================= 学習時と同じ関数を使って初期化用ベクトルを作成する　=========================
    # =========================
    # モデル読み込み
    # =========================
    ...
    embed_initializer = EmbedInitializer(
        ...
        pool_hs_type="mean_pool",
        train_target_category_lst=target_cats,
        propnoun_num_for_init_vec=propnoun_num_for_init_vec, 
        model_name=model_name,
        save_mem_dir=None,
        min_words=min_words,
        max_words=max_words,
    )
    # init_vecs_for_category = embed_initializer.initializeEmbed(
    #     model, 
    #     tokenizer, 
    #     train_token2tokenid, 
    #     init_vec_type, 
    ...
    #     return_init_vecs_only=True, # 初期化vecだけ返すようにする. modelに初期化vecを挿入はしない
    # )  #  {category: [init_vec_for_token1, init_vec_for_token2, ...]} の形式で返ってくるはず
    
    category_to_vecs = {}
    for category, propernouns in category2properNouns.items():
        print(f"\nProcessing category '{category}' with {len(propernouns)} proper nouns for centroid calculation...")
        vecs = embed_initializer.make_initvec_by_wiki_summary_and_hidden_state(
            model=model,
            tokenizer=tokenizer,
            init_terms=propernouns,
            layer_idx=layer_index,
            mix_layers=True,
            return_init_vecs_list_only=True, # カテゴリ内の，複数の既存概念名に対する初期化vecを平均poolする前の、term_vecsのリストを返すようにする
        )
        # NaN / inf 除外
        category_to_vecs[category] = valid_vecs
